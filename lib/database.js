...
export async function saveUserFavorite(userId, jokeId) {
  const config = ensureDatabaseConfig();
  const safeUserId = sanitizeId(userId, 80);
  ...
  if (config.apiUrl) {
    const payload = await requestRemote(config, "/jokes/favorite", {
      method: "POST",
      body: { userId: safeUserId, jokeId: safeJokeId },
    });
    clearJokesCache();
    return payload;
  }
  ...
  const state = await getLocalState(config);
  const joke = state.jokes.find((item) => item.id === safeJokeId);
  if (!joke) {
...
export async function trackJokeView(jokeId, userId = "") {
  const config = ensureDatabaseConfig();
  const safeJokeId = sanitizeId(jokeId, 120);
  const safeUserId = sanitizeId(userId, 80);
