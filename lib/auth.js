...
export async function signupUser(input = {}) {
  ...
  const rawEmail = sanitizeEmail(input.email);
  ...
  const syntheticEmail = phoneNumber
    ? `phone_${phoneNumber.replace(/\D/g, "")}@local.voicejoke.app`
    : "";
  ...
  const exists = state.users.some((user) => {
    ...
    const entryPhone = sanitizePhoneNumber(user?.profile?.phoneNumber || "");
    if (phoneNumber && entryPhone && entryPhone === phoneNumber) {
      return true;
    }
    return false;
  });
