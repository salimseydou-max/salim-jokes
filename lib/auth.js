...
function shouldUseSecureCookieForRequest(config, req) {
...
export function setAuthSessionCookie(res, token, expiresAt, req = null) {
  const config = getAuthConfig();
  const now = Date.now();
  ...
  const secureCookie = shouldUseSecureCookieForRequest(config, req);
...
export function clearAuthSessionCookie(res, req = null) {
  const config = getAuthConfig();
  const secureCookie = shouldUseSecureCookieForRequest(config, req);
